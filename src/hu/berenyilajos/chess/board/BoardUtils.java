...
//    }
    ...
    private Map<String, Integer> initializePositionToCoordinateMap() {
        ...
        for (int i = 0; i < ALGEBRAIC_NOTATION.size(); i++) {
            if (!ALGEBRAIC_NOTATION.get(i).isEmpty()) {
    ...
    public static boolean equals(Map<Integer, Piece> boardConfig, Map<Integer, Piece> boardConfig1) {
        for (Map.Entry<Integer, Piece> b: boardConfig.entrySet()) {
            if (boardConfig1.get(b.getKey()) == null || b.getValue().getPieceType() != boardConfig.get(b.getKey()).getPieceType()) {
                return false;
            }
        }
...
//    }
    ...
    public static boolean isKingPawnTrap(final Board board,
        ...
        return piece != null && piece.is(PieceType.PAWN) &&
               piece.getAlliance() != king.getAlliance();
    }

    public static int mvvlva(final Move move) {
        return move.isAttack() ? move.getAttackedPiece().getPieceValue() - move.getMovedPiece().getPieceValue() : 0;
    }
...
//               Table.get().isRepeatedBoard(board);
//    }
}
