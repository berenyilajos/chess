...
import hu.berenyilajos.chess.gui.Table.MoveLog;
                ...
                if (!line.isEmpty()) {
                    if (isTag(line)) {
                        ...
                        if(!gameText.isEmpty() && gameText.length() > 80) {
                            ...
                            if(game.isValid() && (winner.equals("White") || winner.equals("Black"))) {
                                MySqlGamePersistence.get().persistGame(game);
                                validCount++;
                                System.out.println("(" +(++count)+") Finished parsing " + game);
                            } else {
                                System.err.println("(" +(++count)+") Finished parsing " + game);
                            }
                        }
